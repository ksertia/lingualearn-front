...
// Pagination
const totalPages = computed(() => Math.ceil(store.filteredContenus.length / store.pagination.limit))
const startItem = computed(() => (store.pagination.page - 1) * store.pagination.limit + 1)
const endItem = computed(() => Math.min(store.pagination.page * store.pagination.limit, store.filteredContenus.length))
...
<template>
  <div class="content-table bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
    <!-- Loading State -->
    <div
    ...
    <div v-else>
      <!-- Header -->
      <div class="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr_1fr_100px] px-6 py-4 text-[11px] font-bold uppercase tracking-wider text-gray-400 bg-gray-50 border-b border-gray-100">
        <button @click="handleSort('title')" class="flex items-center gap-1 hover:text-gray-600 transition-colors text-left">
          Contenu {{ getSortIcon('title') }}
        </button>
        <span>Type</span>
        <button @click="handleSort('language')" class="flex items-center gap-1 hover:text-gray-600 transition-colors text-left">
          Langue {{ getSortIcon('language') }}
        </button>
        <button @click="handleSort('level')" class="flex items-center gap-1 hover:text-gray-600 transition-colors text-left">
          Niveau {{ getSortIcon('level') }}
        </button>
        <span>Formateur</span>
        <button @click="handleSort('publishedAt')" class="flex items-center gap-1 hover:text-gray-600 transition-colors text-left">
          Publié {{ getSortIcon('publishedAt') }}
        </button>
        <button @click="handleSort('enrolledCount')" class="flex items-center gap-1 hover:text-gray-600 transition-colors text-left">
          Inscrits {{ getSortIcon('enrolledCount') }}
        </button>
      ...
      <div class="divide-y divide-gray-100">
        <div
          ...
          :key="contenu.id"
          class="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr_100px] items-center px-6 py-4 group transition-all duration-300 hover:bg-gray-50"
        ...
        >
          ...
          <div class="flex items-start gap-3 pr-4">
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2">
                <span class="text-sm font-bold text-gray-900 truncate" :title="contenu.title">
                  {{ contenu.title }}
                </span>
              </div>
              <p class="text-xs text-gray-500 truncate mt-0.5" :title="contenu.description">
                {{ contenu.description }}
              </p>
              <div class="flex items-center gap-2 mt-1">
                <span v-if="contenu.updatedAt" class="text-[10px] text-gray-400">
                  Modifié: {{ formatDate(contenu.updatedAt) }}
                </span>
              </div>
            </div>
          </div>
          ...
          <!-- Langue -->
          <div class="flex items-center gap-2">
            <span class="px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider bg-gray-100 text-gray-700">
              {{ contenu.language.code.toUpperCase() }}
            </span>
          </div>
          ...
          <div class="flex items-center gap-2">
            ...
            <span class="text-xs text-gray-600 truncate" :title="`${contenu.auteur.firstName} ${contenu.auteur.lastName}`">
          ...
          <!-- Inscrits -->
          <div class="flex items-center gap-2">
            <span class="text-sm font-semibold text-gray-900">{{ contenu.stats.enrolledCount }}</span>
            <span v-if="contenu.stats.reportsCount > 0" class="inline-flex items-center justify-center w-5 h-5 rounded-full bg-orange-100 text-[10px] font-bold text-orange-700" :title="`${contenu.stats.reportsCount} signalement(s)`">
            ...
            <div class="relative">
              ...
              >
                ...
                <button
                  ...
                  :class="{ 'opacity-50 cursor-not-allowed': contenu.disabledByGestionnaire }"
                ...
                >
                  ...
                  Réactiver
                  <span v-if="contenu.disabledByGestionnaire" class="text-[10px] text-gray-400 ml-1">(bloqué)</span>
                </button>
      ...
      <!-- Pagination -->
      <div v-if="store.filteredContenus.length > 0" class="flex items-center justify-between px-6 py-4 border-t border-gray-100 bg-gray-50">
        <div class="text-sm text-gray-500">
          Affichage de <span class="font-semibold text-gray-900">{{ startItem }}</span> à 
          <span class="font-semibold text-gray-900">{{ endItem }}</span> sur 
          <span class="font-semibold text-gray-900">{{ store.filteredContenus.length }}</span> résultats
        </div>
