...
const formateurToSuspend = ref<any | null>(null);
...
const handleSuspend = (formateur: any) => {
  formateurToSuspend.value = formateur;
  openMenuId.value = null;
};
...
<template>
  ...
  <div v-if="formateurToSuspend" class="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl p-6 w-full max-w-md">
      <h3 class="text-lg font-semibold text-slate-900 mb-2">Suspendre le formateur</h3>
      <p class="text-sm text-slate-600 mb-6">
        Voulez-vous suspendre {{ formateurToSuspend.firstName }} {{ formateurToSuspend.lastName }} ?
      </p>
      <div class="flex justify-end gap-3">
        <button class="px-4 py-2 rounded-lg border border-slate-200 text-slate-600" @click="formateurToSuspend = null">Annuler</button>
        <button
          class="px-4 py-2 rounded-lg bg-red-600 text-white"
          @click="store.suspendFormateur(formateurToSuspend.id); formateurToSuspend = null"
        >
          Suspendre
        </button>
      </div>
