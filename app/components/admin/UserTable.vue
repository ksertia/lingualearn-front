...
<!-- Header -->
<div
  class="grid grid-cols-[minmax(180px,2.5fr)_minmax(280px,4fr)_1.5fr_2fr_2fr] px-6 py-4 text-[11px] font-bold uppercase tracking-wider text-gray-400"
>
...
<div class="space-y-4">
  <div
    ...
    :key="user.id"
    :class="[
      'grid grid-cols-[2.5fr_3fr_2fr_1.5fr_2fr] items-center border rounded-2xl px-6 py-5 group transition-all duration-300',
      user.isActive
  ...
  >
    ...
    <div class="flex items-center gap-4">
      <div class="relative">
        ...
        <div
          v-if="!user.isActive"
          class="absolute -bottom-1 -right-1 w-5 h-5 rounded-lg bg-[#000099] text-[#00ced1] flex items-center justify-center shadow-md"
    ...
    <!-- Email -->
    <div class="text-sm text-gray-700 truncate" :title="user.email">
      {{ user.email || '-' }}
    </div>
...
const roleTextColor = (role: string) => {
  const map = {
    admin: 'text-[#000099]',
    plateform_manager: 'text-[#f4a261]',
    teacher: 'text-gray-500',
    learner: 'text-[#00ced1]',
    sub_account_learner: 'text-[#00ced1]'
  }
  return map[role as keyof typeof map] || 'text-gray-400'
}
...
const roleIcon = (role: string) => {
  ...
  return icons[role] || icons.learner
}
