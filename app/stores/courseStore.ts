    ...
    async function updateCourse(id: string, data: Partial<Course>) {
        ...
        try {
            ...
            if (courseData) {
                ...
                error.value = response.message || 'Ã‰chec de la mise Ã  jour du cours';
                return false;
            }
        } catch (err: any) {
            error.value = err.data?.message || 'Erreur lors de la mise Ã  jour du cours';
            return false;
        } finally {
            isLoading.value = false;
        }
    ...
    async function deleteCourse(id: string) {
        ...
        try {
            ...
            if (response.success || response === true || !response || Object.keys(response).length === 0) {
                ...
                error.value = response.message || 'Ã‰chec de la suppression du cours';
                return false;
            }
        } catch (err: any) {
